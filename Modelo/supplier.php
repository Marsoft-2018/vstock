<?php
	...
	class Supplier extends ConectarPDO{
		public $id;
		public $bussines_id;
        public $name;
		public $address;
        ...
        public function list(){
            $this->sql="SELECT * FROM suppliers ORDER BY `name` ASC";
			try {
        ...
        public function load(){
            $this->sql="SELECT * FROM suppliers WHERE `id`=? ";  
            try {
				$stm = $this->Conexion->prepare($this->sql);
        ...
        public function add(){ 
			$this->sql = "INSERT INTO suppliers(`id`,`name`,`address`,`phone`,`city`,`email`) VALUES(?,?,?,?,?,?)";
            try {
				$stm = $this->Conexion->prepare($this->sql);
				$stm->bindParam(1, $this->id);
        ...
        public function update(){
            $this->sql = "UPDATE suppliers SET `name` =?,`address` =?,`phone` =?,`city` =?,`email`=? WHERE id = ?";
            try {
				$stm = $this->Conexion->prepare($this->sql);
        ...
        public function delete(){        
            $this->sql="DELETE FROM suppliers WHERE id=? "; 
            try {
				$stm = $this->Conexion->prepare($this->sql);
				$stm->bindParam(1, $this->id);
				if($stm->execute()){
