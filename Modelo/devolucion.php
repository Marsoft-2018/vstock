<?php
	...
	class devolucion extends conectar{
		...
		function ingresarDevolucion($tabla,$factura,$product,$cantidad,$fecha,$cantidadFactura){
			echo "<div class='alert alert-info alert-dismissable'>";
            echo 	"<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>";
            echo 	"Se realizó el registro de la devolucion del Product: $product, Cantidad: $cantidad, Fecha: $fecha";
            echo "</div>";
			...
			$devoluciones=0;
			$cantFinal=0;
			$sqlInventario=mysql_query("SELECT VENTAS,COMPRAS,DEVOLUCIONES,CANT_FINAL FROM inventario WHERE ID_Prod='$product';");
			...
			if($tabla=='devolucionVenta'){	 
	            ...
	            while($pr=mysql_fetch_array($sqlDetalle)){
	            	...
	            	//$sqlActualizarDetalle=mysql_query("UPDATE facturasventasdes SET CANT='".($cantidadFactura-$cantidad)."', subTotal='$subTotal' WHERE idVenta='$factura' AND id_prod='$product';");

            		$sqlActualizarInventario=mysql_query("UPDATE inventario SET VENTAS='".($ventas-$cantidad)."', DEVOLUCIONES='".($devoluciones+$cantidad)."', CANT_FINAL='".($cantFinal+$cantidad)."' WHERE ID_Prod='$product';");
	            }           	
	        }else{	            
	            ...
	            while($pr=mysql_fetch_array($sqlDetalle)){
	            	//$subTotal=($cantidadFactura-$cantidad) * $pr[0];
	            	//$sqlActualizarDetalle=mysql_query("UPDATE facturascomprasdes SET CANT='".($cantidadFactura-$cantidad)."', subTotal='$subTotal' WHERE idCompra='$factura' AND id_prod='$product';");

	            	$sqlActualizarInventario=mysql_query("UPDATE inventario SET COMPRAS='".($compras-$cantidad)."', DEVOLUCIONES='".($devoluciones+$cantidad)."', CANT_FINAL='".($cantFinal-$cantidad)."' WHERE ID_Prod='$product';");
	            }	            
	        }
		}

		function ingresarCambioProduct($modulo,$factura,$productACambiar,$productReemplazo,$cantidad){
            ...
            echo 	"Se realizó el registro del cambio, Tabla: ".$modulo.", Product a cambiar: ".$productACambiar.", Cantidad: ".$cantidad.", Factura: ".$factura.", Product de reemplazo: ".$productReemplazo;
            echo "</div>";
			...
			if($modulo=='devolucionVenta'){	 
				$sqlProductsFacturas=mysql_query("SELECT * FROM facturasventasdes WHERE id_prod='$productACambiar' AND  FACTURA='$factura'");
				$sqlRes=mysql_num_rows($sqlProductsFacturas);

				if($sqlRes > 0){
					while($can=mysql_fetch_array($sqlProductsFacturas)){
						$cantAnterior=$can[3];
						$cantFinal=$cantAnterior-$cantidad;
						$subtotal= $cantFinal * $can[4];
					}
					
					$sqlActualizar=mysql_query("UPDATE facturasventasdes SET CANT='$cantFinal', SubTotal='$subtotal' WHERE id_prod='$productACambiar' AND  FACTURA='$factura'");


				}

				$sqlProductsFacturas2=mysql_query("SELECT * FROM facturasventasdes WHERE id_prod='$productReemplazo' AND  FACTURA='$factura'");
				$sqlRes=mysql_num_rows($sqlProductsFacturas2);

				if($sqlRes > 0){
					while($can=mysql_fetch_array($sqlProductsFacturas2)){
						$cantAnterior=$can[3];
						$cantFinal=$cantAnterior+$cantidad;
						$subtotal= $cantFinal * $can[4];
					}
					$sqlActualizar=mysql_query("UPDATE facturasventasdes SET CANT='$cantFinal', SubTotal='$subtotal' WHERE id_prod='$productReemplazo' AND  FACTURA='$factura'");
				}
			}
			/*
		...
		function datosArticulo($product){

			$sqlArti=mysql_query("SELECT `ARTICULO`,`REFERENCIA`,`PRECIO_COMPRA`,`PRECIO_VENTA` FROM inventario WHERE `ID_Prod`='$product' AND `CANT_FINAL`<>0");
            ...
            echo 		"<strong>Código:</strong> $product, ";
