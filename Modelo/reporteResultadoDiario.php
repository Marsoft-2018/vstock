<?php
            ...
            echo "<h2 style='padding:1px;text-align:center;'>Reporte de Resultados: $dia/".$meses[$mes-1]."/$anho</h2>";
            //Ingresos por ventas de contado y a crédito
                if($dia!=""){
                    
                    $sqlVentaContado=mysql_query("SELECT SUM(fv.TOTAL) AS 'TOTAL' FROM facturasv fv 
                    WHERE  DAY(fv.`Fec_Venta`)='$dia' AND MONTH(fv.`Fec_Venta`)='$mes' AND YEAR(fv.`Fec_Venta`)='$anho'  AND fv.`tipo`='Contado' GROUP BY DAY(fv.`Fec_Venta`);");
                    ...
                    WHERE  DAY(fv.`Fec_Venta`)='$dia' AND MONTH(fv.`Fec_Venta`)='$mes' AND YEAR(fv.`Fec_Venta`)='$anho'  AND fv.`tipo`='Credito' GROUP BY DAY(fv.`Fec_Venta`);");

                    while($t=mysql_fetch_array($sqlVentaCredito)){
                        $totalVentasCredito=$t[0];
                    }

                    $sqlCompraContado=mysql_query("SELECT SUM(fc.TOTAL) AS 'TOTAL' FROM facturasc fc WHERE  DAY(fc.`FECHA`)='$dia' AND MONTH(fc.`FECHA`)='$mes' AND YEAR(fc.`FECHA`)='$anho' AND fc.`tipo`='Contado' GROUP BY DAY(fc.`FECHA`);");
                    $resComprasC=mysql_num_rows($sqlCompraContado);
                    while($tC=mysql_fetch_array($sqlCompraContado)){
                        $totalComprasContado=$tC[0];
                    }

                    $sqlCompraCredito=mysql_query("SELECT SUM(fc.TOTAL) AS 'TOTAL' FROM facturasc fc WHERE DAY(fc.`FECHA`)='$dia' AND  MONTH(fc.`FECHA`)='$mes' AND YEAR(fc.`FECHA`)='$anho' AND fc.`tipo`='Credito' GROUP BY DAY(fc.`FECHA`);");
                    ...
                    while($tCC=mysql_fetch_array($sqlCompraCredito)){
                        ...
                        $sqlTotalDescuentosCompras = mysql_query("SELECT SUM(dc.TOTAL) AS 'TOTAL' FROM descuentocompras dc INNER JOIN facturasc fc ON fc.`FACTURA`=dc.`FACTURA` WHERE   DAY(fc.`FECHA`)='$dia' AND MONTH(fc.`FECHA`)='$mes' AND YEAR(fc.`FECHA`)='$anho' AND fc.`tipo`='Credito' GROUP BY DAY(fc.`FECHA`);");
                        ...
                        if($rsDescuentos>0){
                            while($tDP=mysql_fetch_array($sqlTotalDescuentosCompras)){
                                ...
                                echo "<br>Total descuentos x compras Crédito: $totalDescuentosCompras<br>";
                            }
                        }                      
                    }

                    $sqlAbonosClientes=mysql_query("SELECT SUM(ac.`valorAbono`) AS 'TOTAL' FROM abonos ac WHERE  DAY(ac.`fechaAbono`)='$dia' AND  MONTH(ac.`fechaAbono`)='$mes' AND YEAR(ac.`fechaAbono`)='$anho' GROUP BY MONTH(ac.`fechaAbono`);");
                    ...
                    $sqlAbonosProveedores=mysql_query("SELECT SUM(ap.`Valor_Abono`) AS 'TOTAL' FROM abonosproveedores ap WHERE DAY(ap.`Fecha_Abono`)='$dia' AND  MONTH(ap.`Fecha_Abono`)='$mes' AND YEAR(ap.`Fecha_Abono`)='$anho' GROUP BY DAY(ap.`Fecha_Abono`);");
            ...
            echo "<div class='panel panel-success'>";
                ...
                echo "<div class='panel panel-danger'>";
                        ...
                        $sqlGastos=mysql_query("SELECT g.tipo,SUM(e.VALOR)AS Total FROM gastos g INNER JOIN egresos e ON g.`idGasto`=e.`idGasto` WHERE DAY(e.`FECHA`)='$dia' AND  MONTH(e.`FECHA`)='$mes' AND YEAR(e.`FECHA`)='$anho' GROUP BY DAY(e.`FECHA`);");
                        echo "</div>";
                        echo "<div class='panel-body'>";
                        echo "<div class='row'>";
                        echo    "<div class='col-lg-12'>";
                                    echo "<table class='table table-striped' width='99%' style='border:1px solid; padding:0px;'>";
                                        ...
                                        $sqlTotalGastos=mysql_query("SELECT SUM(e.VALOR)AS Total FROM egresos e WHERE DAY(e.`FECHA`)='$dia' AND  MONTH(e.`FECHA`)='$mes' AND YEAR(e.`FECHA`)='$anho' GROUP BY DAY(e.`FECHA`);");
                        ...
                        echo    "<div class='col-lg-12'>";
                                echo "<div class='panel panel-warning'>";
                                    ...
                                    echo "<table class='table table-striped' width='99%' style='border:1px solid; padding:0px;'>";
                                        ...
                                        echo "</thead>";     
                                        echo "<tbody>";  
                                        $sqlPagosEmpleados=mysql_query("SELECT SUM(pg.VALOR_PAGO)AS Total FROM pagos pg WHERE  DAY(pg.`FECHA_PAGO`)='$dia' AND  MONTH(pg.`FECHA_PAGO`)='$mes' AND YEAR(pg.`FECHA_PAGO`)='$anho' GROUP BY DAY(pg.`FECHA_PAGO`);");
                                        while($pgE=mysql_fetch_array($sqlPagosEmpleados)){
                                            $totalPagosEmpleados= $pgE[0];
                                        }

                                        $sqlListaPagoEmpleados=mysql_query("SELECT em.`NOMBRE1`,em.`NOMBRE2`,em.`APELLIDO1`,em.`APELLIDO2`,SUM(pg.VALOR_PAGO)AS Total FROM pagos pg INNER JOIN empleados em ON em.`ID_EMPLEADO`=pg.`ID_EMPLEADO` WHERE  DAY(pg.`FECHA_PAGO`)='$dia' AND  MONTH(pg.`FECHA_PAGO`)='$mes' AND YEAR(pg.`FECHA_PAGO`)='$anho' GROUP BY em.`ID_EMPLEADO`");
